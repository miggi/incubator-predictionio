...
class JDBCApps(client: String, config: StorageClientConfig, prefix: String)
  ...
  def insert(app: storage.App): Option[Int] = DB localTx { implicit session =>
    val q = if (app.id == 0) {
      sql"""
  ...
  def get(id: Int): Option[storage.App] = DB readOnly { implicit session =>
