...
import org.apache.predictionio.{StorageClientConfig, storage}
import org.apache.predictionio.data.storage.App
import org.apache.predictionio.storage.Apps
...
class ESApps(client: Client, config: StorageClientConfig, index: String)
  ...
  def insert(app: storage.App): Option[Int] = {
    val id =
      if (app.id == 0) {
  ...
  def get(id: Int): Option[storage.App] = {
    try {
      val response = client.prepareGet(
        index,
        estype,
        id.toString).get()
      Some(read[storage.App](response.getSourceAsString))
    } catch {
  ...
  def getAll(): Seq[storage.App] = {
    try {
      val builder = client.prepareSearch(index).setTypes(estype)
      ESUtils.getAll[storage.App](client, builder)
    } catch {
      case e: ElasticsearchException =>
        error(e.getMessage)
        Seq[storage.App]()
  ...
  def update(app: storage.App): Unit = {
    try {
